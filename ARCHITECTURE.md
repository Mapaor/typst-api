...
## Fonts
The server loads global fonts via a dedicated font database at startup using the `fontdb` crate. This database can be updated dynamically via a specific endpoint (`/fonts/refresh`).
...
The API can also be made private or secure using optional bearer tokens.
